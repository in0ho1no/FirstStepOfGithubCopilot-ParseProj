...
### 5.5 コンパイラ組み込みマクロ allow-list

`.mtpj` に載らないコンパイラ組み込みマクロを外部 JSON で持つ。

**ファイル名**: `compiler_builtins.json`（`mtpj_deps.py` と同階層に配置。存在しなければ空辞書扱い）
...
```json
{
  ...
  "__TARGET_ARCH__": "1",
  "__VENDOR__": "1",
  "__COMPILER_VERSION__": "0x01000000"
}
```

- 利用者が使用するコンパイラに合わせて編集できる。
...
| `compiler_builtins.json`  | コンパイラ組み込みマクロ allow-list（最低限のサンプル同梱） |
